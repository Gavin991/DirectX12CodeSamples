...
float4 tex01 = shaderTexture.Sample(SampleType, input.uv);// *float4(input.color, 1.0f);//
float4 tex02 =  shaderTexture01.Sample(SampleType, input.uv);// *float4(input.color, 1.0f);//

textureColor=lerp(tex01, tex02, 0.5f);
