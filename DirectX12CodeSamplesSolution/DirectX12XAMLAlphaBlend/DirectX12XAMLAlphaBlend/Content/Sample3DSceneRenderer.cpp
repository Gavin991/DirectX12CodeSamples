...
using namespace DirectX12XAMLAlphaBlend;
...
bool Sample3DSceneRenderer::Render()
{
	...
	PIXBeginEvent(m_commandList.Get(), 0, L"Draw the cube");
	{
		...
		//使用第三张纹理，渲染第一个立方体
		m_textureManager.BindTexture(&m_cbvHeapManager, L"Assets\\brick_wall.jpg", 1, m_commandList);		
		if (m_tracking)
		{
			...
			curCubeMatrix = XMMatrixRotationY(radians)*XMMatrixScaling(0.8f, 0.8f, 0.8f)*XMMatrixTranslation(0, -0.7f, 0);
		}
		else
		{
			curCubeMatrix = XMMatrixRotationY(-m_angle*0.5f)*XMMatrixScaling(0.8f, 0.8f, 0.8f)*XMMatrixTranslation(0, -0.7f, 0);
