...
namespace DirectX12FbxBase
{
	...
	void TextureManager::LoadTexture(ConstantBufferViewHeapManager *cbvHeapManager, std::wstring szFile, Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList> commandList)
	{
		map<std::wstring, shared_ptr<TextureInformationStruct>>::iterator it;
...
#pragma region 加载纹理部分
			//LPCWSTR szFile02 = szFile;

			DWORD flags = DirectX::CP_FLAGS::CP_FLAGS_NONE;
			ScratchImage image;
			DirectX::LoadFromWICFile(szFile.c_str(), flags, nullptr, image);
...
#pragma endregion
			//int strSize = wcslen(szFile)+1;
			//wchar_t *pathStr =new wchar_t[strSize];
			//ZeroMemory(pathStr, strSize * sizeof(wchar_t));
			//
			//for (int index = 0; index<strSize; index++)
			//{
			//	pathStr[index] = szFile[index];
			//}
			//pathStr[strSize] = L'\0';
			//szFile02 = pathStr;
			//wcscpy_s(pathStr, strSize, szFile02);
			m_textureMap.insert(pair<std::wstring, shared_ptr<TextureInformationStruct>>(szFile, tempStruct));
		//	TCHAR  sOut[512];
	...
	void TextureManager::RemoveUselessContent()
	{
		...
		for (map<std::wstring, shared_ptr<TextureInformationStruct>>::iterator it = m_textureMap.begin(); it != m_textureMap.end(); ++it)
			it->second->m_pTextureUpload.Reset();
	}

	void  TextureManager::BindTexture(ConstantBufferViewHeapManager *cbvHeapManager, std::wstring szFile, UINT rootParameterIndex, Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList> commandList)
	{
		map<std::wstring, shared_ptr<TextureInformationStruct>>::iterator it;
		it = m_textureMap.find(szFile);
		if (it != m_textureMap.end())
	...
	void TextureManager::LoadDDsTexture(ConstantBufferViewHeapManager *cbvHeapManager, std::wstring szFile, Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList> commandList, D3D12_SRV_DIMENSION srvType)
	{	
		map<std::wstring, shared_ptr<TextureInformationStruct>>::iterator it;
		it = m_textureMap.find(szFile);
		if (it == m_textureMap.end())
