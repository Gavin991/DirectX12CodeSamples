...
#pragma region 创建管线状态部分
			// 通过异步方式加载着色器。
			auto createVSTask01 = DX::ReadDataAsync(L"SampleVertexShader.cso").then([this, tempNormalMapStruct](std::vector<byte>& fileData) {
				tempNormalMapStruct->m_vertexShader = fileData;
			});
			//CubeMapRefractPixelShader CubeMapRefractPixelShader
			auto createPSTask01 = DX::ReadDataAsync(L"SamplePixelShader.cso").then([this, tempNormalMapStruct](std::vector<byte>& fileData) {
...
#pragma endregion
			...
			createPipelineStateTask.then([this, deviceResources, callBackFun, tempNormalMapStruct, tempCubeMapStruct, tempLightStruct]()
			{
				...
				m_pipelineStateMap.insert(pair<LPCWSTR, shared_ptr<PipelineStateInformationStruct>>(L"LightHLSLShader", tempNormalMapStruct));

				m_pipelineStateMap.insert(pair<LPCWSTR, shared_ptr<PipelineStateInformationStruct>>(L"CubeMapHLSLShader", tempCubeMapStruct));

				m_pipelineStateMap.insert(pair<LPCWSTR, shared_ptr<PipelineStateInformationStruct>>(L"PlaneHLSLShader", tempLightStruct));
