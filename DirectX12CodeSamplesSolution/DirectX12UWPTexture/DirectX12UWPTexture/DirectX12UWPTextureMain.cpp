...
// 加载应用程序时加载并初始化应用程序资产。
DirectX12UWPTextureMain::DirectX12UWPTextureMain()
{
...
// 创建并初始化呈现器。
void DirectX12UWPTextureMain::CreateRenderers(const std::shared_ptr<DX::DeviceResources>& deviceResources)
{
...
// 每帧更新一次应用程序状态。
void DirectX12UWPTextureMain::Update()
{
	// 更新场景对象。
	m_timer.Tick([&]()
	{
...
bool DirectX12UWPTextureMain::Render()
{
	// 在首次更新前，请勿尝试呈现任何内容。
...
void DirectX12UWPTextureMain::OnWindowSizeChanged()
{
...
// 向应用通知它将挂起。
void DirectX12UWPTextureMain::OnSuspending()
{
	// TODO: 将此内容替换为应用的挂起逻辑。
...
// 向应用通知它将不再挂起。
void DirectX12UWPTextureMain::OnResuming()
{
	// TODO: 将此内容替换为应用的恢复逻辑。
}

// 通知呈现器，需要释放设备资源。
void DirectX12UWPTextureMain::OnDeviceRemoved()
{
