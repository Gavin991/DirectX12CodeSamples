...
#pragma region 创建管线状态部分
			// 通过异步方式加载着色器。
			auto createVSTask01 = DX::ReadDataAsync(L"TwoLayerTextureVertexShader.cso").then([this, tempStruct01](std::vector<byte>& fileData) {
				tempStruct01->m_vertexShader = fileData;
			});

			auto createPSTask01 = DX::ReadDataAsync(L"TwoLayerTexturePixelShader.cso").then([this, tempStruct01](std::vector<byte>& fileData) {
				tempStruct01->m_pixelShader = fileData;
			});

			auto createGSTask01 = DX::ReadDataAsync(L"TwoLayerTextureGeometryShader.cso").then([this, tempStruct01](std::vector<byte>& fileData) {
				tempStruct01->m_geometryShader = fileData;
			});
			//-------------------------------
			...
			auto createGSTask02 = DX::ReadDataAsync(L"PlaneGeometryShader.cso").then([this, tempStruct02](std::vector<byte>& fileData) {
				tempStruct02->m_geometryShader = fileData;
			});

			// 加载着色器之后创建管道状态。
			auto createPipelineStateTask = (createVSTask01 && createPSTask01 && createGSTask01 && createVSTask02 && createPSTask02 && createGSTask02).then([this, rootSignature, deviceResources, tempStruct01, tempStruct02]() {
				...
				state.GS = CD3DX12_SHADER_BYTECODE(&tempStruct01->m_geometryShader[0], tempStruct01->m_geometryShader.size());
				...
				CD3DX12_RASTERIZER_DESC tempheiha = CD3DX12_RASTERIZER_DESC(D3D12_DEFAULT);
				tempheiha.FillMode = D3D12_FILL_MODE_WIREFRAME;
				...
				state02.GS = CD3DX12_SHADER_BYTECODE(&tempStruct02->m_geometryShader[0], tempStruct02->m_geometryShader.size());
				state02.RasterizerState = tempheiha;//CD3DX12_RASTERIZER_DESC(D3D12_DEFAULT);//
