...
using namespace DirectX12XAMLGeometry_Shader;
	...
	m_deviceResources(deviceResources)
{
	...
	ZeroMemory(&m_GSCBData, sizeof(m_GSCBData));
...
Sample3DSceneRenderer::~Sample3DSceneRenderer()
{
	...
	if (m_GSCB != nullptr)
	{
		m_GSCB->Unmap(0, nullptr);
	}
	m_mappedGSCB = nullptr;
}
...
void Sample3DSceneRenderer::CreateDeviceDependentResources()
{
	...
	// 创建具有单个常量缓冲区槽的根签名。
	{
		...
		range5.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_CBV;
		range5.NumDescriptors = 1;
		range5.BaseShaderRegister = 0;
		...
		parameter5.ShaderVisibility = D3D12_SHADER_VISIBILITY_GEOMETRY;
...
void Sample3DSceneRenderer::CreateWindowSizeDependentResources()
{
	...
	//---------------------
	XMStoreFloat4x4(&m_GSCBData.projection, modelMatrix);
	XMStoreFloat4x4(&m_GSCBData.view, viewMatrix);
	m_GSCBData.explode = 0.5f;
	m_GSCBData.cameraPosition = DirectX::XMFLOAT3(0.0f, 0.7f, 1.5f);
	m_GSCBData.lightDirction = DirectX::XMFLOAT3(-1.0f, -1.0f, -1.0f);
	//XMStoreFloat4x4(&m_GSCBData.model, XMMatrixTranspose(XMMATRIX));
	//------------------------
}
...
void Sample3DSceneRenderer::Update(DX::StepTimer const& timer)
{
	...
	////m_GSCBData.lightDirction = DirectX::XMFLOAT3(m_lightX, -1.0f, -1.0f);
	//m_GSCBData.explode = 0.5f;
	//m_GSCBData.cameraPosition = DirectX::XMFLOAT3(0.0f, 0.7f, 1.5f);
	//m_GSCBData.lightDirction = DirectX::XMFLOAT3(-1.0f, -1.0f, -1.0f);
...
bool Sample3DSceneRenderer::Render()
{
	...
	//DX::ThrowIfFailed(m_commandList->Reset(m_deviceResources->GetCommandAllocator(), m_pipelineStateManager.GetPipelineStageByName(L"SampleHLSLShader").Get()));
	//m_commandList->SetPipelineState(m_pipelineStateManager.GetPipelineStageByName(L"SampleHLSLShader").Get());
	...
	//Microsoft::WRL::ComPtr<ID3D12PipelineState> second = m_pipelineStateManager.GetPipelineStageByName(L"PlaneHLSLShader");
	...
	PIXBeginEvent(m_commandList.Get(), 0, L"Draw the cube");
	{
		...
		//使用第一张纹理，渲染第一个球体
		m_textureManager.BindTexture(&m_cbvHeapManager, L"Assets\\pic03.jpg", 1, m_commandList);	
		
		//m_textureManager.BindTexture(&m_cbvHeapManager, L"Assets\\pic02.jpg", 1, m_commandList);
		curCubeMatrix = XMMatrixScaling(5.0f, 0.05f, 5.0f)*XMMatrixTranslation(0, -0.7f, 0);

		//-----------------
		// 更新几何着色器常量缓冲区资源。
		XMStoreFloat4x4(&m_GSCBData.model, XMMatrixTranspose(curCubeMatrix));
		m_GSCBData.explode = m_explode;
		m_GSCBData.cameraPosition = DirectX::XMFLOAT3(m_fCameraX, m_fCameraY, 1.5f);
		m_GSCBData.lightDirction = DirectX::XMFLOAT3(m_lightX, -1.0f, -1.0f);
		UINT8* destination = m_mappedGSCB + (m_deviceResources->GetCurrentFrameIndex() * c_AlignedGSCBSize);
		memcpy(destination, &m_GSCBData, sizeof(m_GSCBData));
		CD3DX12_GPU_DESCRIPTOR_HANDLE gpuHandle(m_cbvHeapManager.GetGPUDescriptorHandleForHeapStart(), m_iStartOffset + m_deviceResources->GetCurrentFrameIndex(), m_cbvHeapManager.GetCbvDescriptorSize());
		m_commandList->SetGraphicsRootDescriptorTable(4, gpuHandle);
		//-------------------
		...
		////使用第一张纹理，渲染第一个球体
		m_textureManager.BindTexture(&m_cbvHeapManager, L"Assets\\pic01.jpg", 1, m_commandList);
		m_textureManager.BindTexture(&m_cbvHeapManager, L"Assets\\pic04.png", 3, m_commandList);
		DirectX::XMFLOAT3 offsetPos = { 1,0,0 };
	...
	//	m_textureManager.BindTexture(&m_cbvHeapManager, L"Assets\\pic04.png", 3, m_commandList);
		...
		if (m_tracking)
		{
			...
			///*Rotate(radians);*/XMStoreFloat4x4(&m_constantBufferData.model, XMMatrixTranspose(XMMatrixRotationY(radians)));
			curCubeMatrix = XMMatrixRotationY(radians)*XMMatrixScaling(0.5f, 0.5f, 0.5f);			
		}
		else
		{
...
void Sample3DSceneRenderer::OnHandleCallBack(bool res)
{
	...
	auto createAssetslambda = [this]() {
		...
		CD3DX12_RESOURCE_DESC constantBufferDesc = CD3DX12_RESOURCE_DESC::Buffer((DX::c_frameCount)  * c_AlignedGSCBSize);
		...
		NAME_D3D12_OBJECT(m_GSCB);

		// 创建常量缓冲区视图以访问上载缓冲区。
		D3D12_GPU_VIRTUAL_ADDRESS cbvGpuAddress = m_GSCB->GetGPUVirtualAddress();
		m_iStartOffset = m_cbvHeapManager.GetHeapCurrentOffset();
		m_cbvDescriptorSize = m_cbvHeapManager.GetCbvDescriptorSize();
		for (UINT n = 0; n < DX::c_frameCount; n++)
		{
			m_cbvHeapManager.CreateConstantBufferView(cbvGpuAddress, c_AlignedGSCBSize);
		}

		// 映射常量缓冲区。
		CD3DX12_RANGE readRange(0, 0);		// 我们不打算从 CPU 上的此资源中进行读取。
		DX::ThrowIfFailed(m_GSCB->Map(0, &readRange, reinterpret_cast<void**>(&m_mappedGSCB)));
		//DX::ThrowIfFailed(m_constantBuffer->Map(0, nullptr, reinterpret_cast<void**>(&m_mappedConstantBuffer)));

		ZeroMemory(m_mappedGSCB, DX::c_frameCount * c_AlignedGSCBSize);
...
#pragma region 加载纹理部分
		...
		m_textureManager.LoadTexture(&m_cbvHeapManager, L"Assets\\pic01.jpg", m_commandList);
#pragma endregion
...
#pragma endregion
		...
		m_textureManager.LoadTexture(&m_cbvHeapManager, L"Assets\\pic02.jpg", m_commandList);
		m_textureManager.LoadTexture(&m_cbvHeapManager, L"Assets\\pic03.jpg", m_commandList);
		m_textureManager.LoadTexture(&m_cbvHeapManager, L"Assets\\pic04.png", m_commandList);
