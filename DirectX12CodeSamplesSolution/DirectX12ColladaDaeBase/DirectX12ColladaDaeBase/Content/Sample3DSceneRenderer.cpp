...
using namespace DirectX12ColladaDaeBase;
...
bool Sample3DSceneRenderer::Render()
{
	...
	PIXBeginEvent(m_commandList.Get(), 0, L"Draw the cube");
	{
		...
		curCubeMatrix = XMMatrixScaling(5.0f, 0.05f, 5.0f)*XMMatrixTranslation(0, -0.9f, 0);
		...
		m_textureManager.BindTexture(&m_cbvHeapManager, L"Assets\\banner.tif", 1, m_commandList);//banner.tif props1.tif//
		curCubeMatrix = XMMatrixRotationY(-m_angle*1.5f)*XMMatrixScaling(0.5f, 0.5f, 0.5f)*XMMatrixTranslation(0, -0.9f, -2);
		m_entity3d.UpdateEntity3DByIndex(&m_cbvHeapManager, 0, m_deviceResources->GetCurrentFrameIndex(), curCubeMatrix);
		m_entity3d.DrawEntity3D(m_deviceResources->GetCurrentFrameIndex());
...
void Sample3DSceneRenderer::OnHandleCallBack()
{
	...
	auto createAssetslambda = [this]() {
		...
		m_entity3d.Initialize(d3dDevice, m_commandList, 1);
		//m_entity3d.CreateEntity3DVector();
		m_entity3d.CreateEntity3D(&m_collada);
		m_entity3d.UpdateVertexAndIndexSubresources();

		

		// 为常量缓冲区创建描述符堆。
		{
			int custom_geometry_num = m_CustomCube.GetCubeNumber() + m_customPlane.GetPlaneNumber() +m_entity3d.GetEntity3DNumber();
		...
		m_entity3d.UpLoadVextureConstantBuffer(&m_cbvHeapManager);
		m_entity3d.CreateBufferView();
