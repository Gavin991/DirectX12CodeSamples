...
float4 main(PixelShaderInput input) : SV_TARGET
{
	...
	//textureColor =( float4(ambientColor, 1.0f) + float4(diffuseColor, 1.0f)*float4(input.color, 1.0f))* textureColor;
	float3 colorb = input.color + ambientColor;
	//textureColor = float4(colorb, 1.0f)*textureColor;
	//return textureColor;
