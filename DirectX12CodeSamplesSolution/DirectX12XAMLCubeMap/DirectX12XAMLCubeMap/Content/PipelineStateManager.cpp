...
#pragma region 创建管线状态部分
			// 通过异步方式加载着色器。
			auto createVSTask01 = DX::ReadDataAsync(L"CubeMapReflectVertexShader.cso").then([this, tempStruct01](std::vector<byte>& fileData) {
				tempStruct01->m_vertexShader = fileData;
			});
			//CubeMapRefractPixelShader CubeMapRefractPixelShader
			auto createPSTask01 = DX::ReadDataAsync(L"CubeMapFresnelPixelShader.cso").then([this, tempStruct01](std::vector<byte>& fileData) {
