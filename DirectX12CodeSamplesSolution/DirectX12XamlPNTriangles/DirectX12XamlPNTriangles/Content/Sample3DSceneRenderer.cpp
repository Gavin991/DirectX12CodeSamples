...
using namespace DirectX12XamlPNTriangles;
...
void Sample3DSceneRenderer::CreateDeviceDependentResources()
{
	...
	// 创建具有单个常量缓冲区槽的根签名。
	{
		...
		parameter1.ShaderVisibility = D3D12_SHADER_VISIBILITY_DOMAIN;//D3D12_SHADER_VISIBILITY_VERTEX;
...
bool Sample3DSceneRenderer::Render()
{
	...
	PIXBeginEvent(m_commandList.Get(), 0, L"Draw the cube");
	{
		...
		//CD3DX12_GPU_DESCRIPTOR_HANDLE gpuHandle(m_cbvHeapManager.GetGPUDescriptorHandleForHeapStart(), m_deviceResources->GetCurrentFrameIndex(), m_cbvHeapManager.GetCbvDescriptorSize());
		...
		//gpuHandle.InitOffsetted(m_cbvHeapManager.GetGPUDescriptorHandleForHeapStart(), DX::c_frameCount+3, m_cbvHeapManager.GetCbvDescriptorSize());
		//CD3DX12_GPU_DESCRIPTOR_HANDLE gpuHandle(m_cbvHeapManager.GetGPUDescriptorHandleForHeapStart(), DX::c_frameCount + 3, m_cbvHeapManager.GetCbvDescriptorSize());
		...
		DirectX::XMMATRIX curCubeMatrix = XMMatrixRotationY(-m_angle*0.5f)*XMMatrixScaling(0.5f, 0.5f, 0.5f);
