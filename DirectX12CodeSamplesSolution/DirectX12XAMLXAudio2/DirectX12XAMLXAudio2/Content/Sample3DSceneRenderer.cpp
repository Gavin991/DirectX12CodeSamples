...
void Sample3DSceneRenderer::CreateDeviceDependentResources()
{
	...
	auto targetHitSound = mediaReader->LoadMedia("Assets\\背景音乐.wav");//hit.wav

	m_bgSound = ref new SoundEffect();
	m_bgSound->Initialize(
		m_audioController->MusicEngine(),//m_audioController->SoundEffectEngine(),
		mediaReader->GetOutputWaveFormatEx(),
		targetHitSound	);

	m_bgSound->PlaySound(1.0f);
}
...
void Sample3DSceneRenderer::Update(DX::StepTimer const& timer)
{
	...
	if (m_loadingComplete)
	{
		...
		if (m_bgSound!=nullptr)
		{
			DirectX::XMFLOAT3 eyePosition = { m_fCameraX,m_fCameraY, 1.5f };
			//cubePosition
			float dis = (cubePosition.x - m_fCameraX)*(cubePosition.x - m_fCameraX)
				+(cubePosition.y - m_fCameraY)*(cubePosition.y - m_fCameraY)
				+ (cubePosition.z - 1.5f)*(cubePosition.z - 1.5f);
			dis = sqrt(dis);

			float flashVolume = 1.0f;
			if (dis>5.5)
			{
				flashVolume = 0;
			}
			...
			{
				dis = dis - 0.5f;
				flashVolume = 1 - dis / 5.0f;
			}
			m_bgSound->SetVolume(flashVolume);
		}
...
bool Sample3DSceneRenderer::Render()
{
	...
	PIXBeginEvent(m_commandList.Get(), 0, L"Draw the cube");
	{
		...
		if (m_tracking)
		{
			...
			curCubeMatrix = XMMatrixRotationY(radians)*XMMatrixScaling(0.5f, 0.5f, 0.5f)*XMMatrixTranslation(cubePosition.x, cubePosition.y, cubePosition.z);	//	0.5f, 0.5f, 0.5f	
		}
		else
		{
			curCubeMatrix = XMMatrixRotationY(-m_angle*0.5f)*XMMatrixScaling(0.5f, 0.5f, 0.5f)*XMMatrixTranslation(cubePosition.x, cubePosition.y, cubePosition.z);
		}
		//curCubeMatrix = cubePosition*XMMatrixTranslation(cubePosition.x, cubePosition.y, cubePosition.z);
		//curCubeMatrix = XMMatrixScaling(10.0f, 10.0f, 10.0f);

		m_CustomCube.UpdateCubeByIndex(&m_cbvHeapManager, 1, m_deviceResources->GetCurrentFrameIndex(), curCubeMatrix);
...
void Sample3DSceneRenderer::OnHandleCallBack(bool res)
{
	...
	auto createAssetslambda = [this]() {
		...
		m_CustomCube.Initialize(d3dDevice, m_commandList, 2);
		...
		// 为常量缓冲区创建描述符堆。
		{
			...
			int numDescriptos = DX::c_frameCount*custom_geometry_num + 5 + 1;
...
void Sample3DSceneRenderer::HandleCubeMove(int way)
{
	if (way==0)
	{
		cubePosition.z -= 0.1f;
