...
void Sample3DSceneRenderer::CreateDeviceDependentResources()
{
	...
	// 创建具有单个常量缓冲区槽的根签名。
	{
		...
		//------------cumpute shaser-----------start
		BuildPostProcessRootSignature();
	}

	//创建图形管线状态
	auto callBackFun = std::bind(&Sample3DSceneRenderer::OnHandleCallBack, this, std::placeholders::_1);
	m_pipelineStateManager.CreateGraphicsPipelineState(m_rootSignature, mPostProcessRootSignature, m_deviceResources, callBackFun);
...
void Sample3DSceneRenderer::CreateWindowSizeDependentResources()
{
	...
	auto d3dDevice = m_deviceResources->GetD3DDevice();
	if (mGrayFilter != nullptr)
	{
		mGrayFilter->OnResize(static_cast<UINT>(viewport.Width), static_cast<UINT>(viewport.Height));
	}
	else
	{
		mGrayFilter = std::make_unique<GrayGilter>(d3dDevice,
			static_cast<UINT>(viewport.Width), static_cast<UINT>(viewport.Height), DXGI_FORMAT_R8G8B8A8_UNORM);
	}
	//---------------------------------------compute shader------------------------end
}
...
bool Sample3DSceneRenderer::Render()
{
	...
	PIXBeginEvent(m_commandList.Get(), 0, L"Draw the cube");
	{
		...
		//----------------------------------------compute sader--------------------start
		mGrayFilter->GrayBehavior(
			m_commandList.Get(),
			mPostProcessRootSignature.Get(),
			m_pipelineStateManager.GetPipelineStageByName(L"GrayComputeShader").Get(),	
			m_deviceResources->GetRenderTarget());

		// Prepare to copy blurred output to the back buffer.
		m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_deviceResources->GetRenderTarget(),
			D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_COPY_DEST));

		m_commandList->CopyResource(m_deviceResources->GetRenderTarget(), mGrayFilter->Output());
...
void Sample3DSceneRenderer::OnHandleCallBack(bool res)
{
	...
	auto createAssetslambda = [this]() {
		...
		// 为常量缓冲区创建描述符堆。
		{
			...
			//int numDescriptos = DX::c_frameCount*custom_geometry_num //各个3D物体的常量缓存
			//	+ 1//纹理（三张）的	
			//	+ 1//像素着色器所以的常量缓存
			//	+4;
			int numDescriptos = DX::c_frameCount*custom_geometry_num //各个3D物体的常量缓存
...
#pragma endregion
		...
		//
		int custom_geometry_num02 = m_customSphere.GetSphereNumber() + m_CustomCube.GetCubeNumber() + m_customPlane.GetPlaneNumber();
		//int numDescriptos02 = DX::c_frameCount*custom_geometry_num02 + 1 + 1 + 4;
		int numDescriptos02 = DX::c_frameCount*custom_geometry_num02 +6;
		mGrayFilter->BuildDescriptorsForGray(
			CD3DX12_CPU_DESCRIPTOR_HANDLE(m_cbvHeapManager.GetCbvHeap().Get()->GetCPUDescriptorHandleForHeapStart(), numDescriptos02, m_cbvHeapManager.GetCbvDescriptorSize()),
			CD3DX12_GPU_DESCRIPTOR_HANDLE(m_cbvHeapManager.GetCbvHeap().Get()->GetGPUDescriptorHandleForHeapStart(), numDescriptos02 , m_cbvHeapManager.GetCbvDescriptorSize()),
			m_cbvHeapManager.GetCbvDescriptorSize());
		//----------------------------compute shader--------------------end
...
//------------cumpute shaser-----------start
void Sample3DSceneRenderer::BuildPostProcessRootSignature()
{
	...
	auto d3dDevice = m_deviceResources->GetD3DDevice();
	DX::ThrowIfFailed(d3dDevice->CreateRootSignature(
		0,
