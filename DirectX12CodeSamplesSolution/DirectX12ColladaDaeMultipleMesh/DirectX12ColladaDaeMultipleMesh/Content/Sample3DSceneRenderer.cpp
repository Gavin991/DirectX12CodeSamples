...
#include <iostream>
using namespace DirectX12ColladaDaeMultipleMesh;
...
Sample3DSceneRenderer::~Sample3DSceneRenderer()
{
	...
	//m_entity3d.ClearConstantBuffer();
	m_scene3d.ClearScene3DConstantBuffer();
}
...
void Sample3DSceneRenderer::CreateDeviceDependentResources()
{
	...
	Platform::String^ name = L"Assets\\katongjiuwu.xml";//Banner.xml Tow_Gauss3 banner.xml box.xml Prop_Barrel.xml katongjiuwu.xml
	m_collada.LoadFromDBE(name,	callBackFunForDBE);
...
void Sample3DSceneRenderer::CreateWindowSizeDependentResources()
{
	...
	g_SimpleCamera.SetProjectionParameters(fovAngleY, aspectRatio, 0.01f, 100000.0f);
	...
	//m_entity3d.SetModelViewMatrix(modelMatrix, viewMatrix);
	m_scene3d.SetScene3DViewMatrix(modelMatrix, viewMatrix);
...
bool Sample3DSceneRenderer::Render()
{
	...
	PIXBeginEvent(m_commandList.Get(), 0, L"Draw the cube");
	{
		...
		curCubeMatrix = XMMatrixRotationY(-m_angle*1.5f)*XMMatrixScaling(0.01f, 0.01f, 0.01f)*XMMatrixTranslation(0, -2.0f, -3);
		...
		curCubeMatrix = XMMatrixScaling(20.0f, 20.0f, 20.0f);
...
void Sample3DSceneRenderer::OnHandleCallBack()
{
	...
	auto createAssetslambda = [this]() {
		...
		m_scene3d.CreatScene3D(d3dDevice, m_commandList, &m_collada);
		m_scene3d.UpdateScene3DSubresources();

		// 为常量缓冲区创建描述符堆。
		{
			int custom_geometry_num = m_CustomCube.GetCubeNumber() + m_customPlane.GetPlaneNumber()  + m_scene3d.GetAllEnity3DNumber();//+m_entity3d.GetEntity3DNumber()
		...
		m_scene3d.UpLoadScene3DConstantBuffer(&m_cbvHeapManager);
		m_scene3d.CreateScene3DBufferView();
...
#pragma endregion
	...
	createAssetsTask.then([this]() {
		...
		//m_entity3d.RemoveUselessContent();
		m_scene3d.RemoveScene3DUselessContent();
	});
}