	...
	void* pUserContext)
{
	...
	g_SimpleCamera.SetViewParameters(XMFLOAT3(0.0f, 6.0f, -6.0f), XMFLOAT3(0.0f, 1.0f, 0.0f), XMFLOAT3(0.0f, 1.0f, 0.0f));
	...
	double fTime, float fElapsedTime, void* pUserContext)
{
	...
	g_World= XMMatrixRotationY(t)*XMMatrixTranslation(10, 4, 10);
	mWorldViewProjection = g_World * g_View * g_Projection;	
	XMStoreFloat4x4(&m, mWorldViewProjection);
	g_pmWorldViewProjection->SetMatrix((float*)&m);
	g_ptxDiffuse->SetResource(g_pTextureRV);
	for (UINT p = 0; p < techDesc.Passes; ++p)
	...
	g_World = XMMatrixTranslation(-10, 4, 30)*XMMatrixRotationY(t);
	mWorldViewProjection = g_World * g_View * g_Projection;
	XMStoreFloat4x4(&m, mWorldViewProjection);
	g_pmWorldViewProjection->SetMatrix((float*)&m);
	g_ptxDiffuse->SetResource(g_pTextureRV);
...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	InitApp();
	DXUTCreateWindow(L"DrectXSeven");
